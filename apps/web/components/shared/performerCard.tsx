...
export default function PerformerCard({ performer, variant = 'robroy' }: PerformerCardProps) {
    ...
    return (
        <motion.div
            ...
            viewport={{ once: true }}
            className={`group rounded-2xl overflow-hidden transition-all duration-500 ${styles.performerCard.container} ${styles.performerCard.shadow}`}
        >
            {/* Image section */}
            <div className="relative aspect-3/4 overflow-hidden">
                ...
                />
                <div className={`absolute inset-0 ${styles.performerCard.imageOverlay}`} />

                {/* Type badge - top left */}
                <div className="absolute top-3 left-3">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider backdrop-blur-sm ${styles.performerCard.typeBadge}`}>
                ...
                {performer.nextDate && (
                    <div className="absolute top-3 right-3">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium backdrop-blur-sm ${styles.performerCard.nextBadge}`}>
            ...
            <div className="p-5">
                {/* Name & genre */}
                <h3 className={`text-xl font-bold mb-1 ${styles.performerCard.nameText}`}>
                    {performer.name}
                </h3>
                <p className={`text-sm font-medium mb-3 ${styles.performerCard.genreText}`}>
                    {performer.genre}
                </p>

                {/* Bio */}
                {performer.bio && (
                    <p className={`text-sm leading-relaxed mb-4 line-clamp-2 ${styles.performerCard.bioText}`}>
                        {performer.bio}
                    </p>
                )}

                {/* Divider */}
                <div className={`border-t ${styles.performerCard.divider} pt-4`}>
                    {/* Bottom row: social links + next venue */}
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            {performer.instagram && (
                                <a
                                    ...
                                    className={`p-2 rounded-lg transition-all duration-300 ${styles.performerCard.socialButton} ${styles.performerCard.socialButtonHover}`}
                        ...
                        {/* Next venue hint */}
                        {performer.nextVenue && (
                            <span className={`inline-flex items-center gap-1 text-xs ${styles.performerCard.genreText}`}>
